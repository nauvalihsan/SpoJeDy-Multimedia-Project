  ...
  >
    ...
    <main class="ml-[250px] flex-1 px-8 pt-6 relative">
      ...
      <div v-if="searchQuery" class="relative z-10">
        ...
        <div v-if="filteredSongs.length" class="grid grid-cols-3 gap-6">
          <VideoItem
        ...
        <p v-else class="text-sm opacity-60">No videos found for "{{ searchQuery }}"</p>
      </div>

      <!-- Normal View -->
      <div v-else class="relative z-10">
        <h1 class="text-3xl font-black mb-4">Music Video</h1>
...
// Play video - goes to VideoDetail page
function playVideo(song) {
  router.push(`/video/${song.id}`) // Player in bottom bar keeps playing audio
}
</script>
